...
| 코드 | 설명 |
|------|------|
| `<link href="bootstrap...">` | Bootstrap CSS 불러오기 (예쁜 스타일 자동 적용) |
| `id="time-input"` | 뽀모도로 타이머 입력창 (JS에서 찾을 용도) |
| `id="todo-input"` | 할 일 입력창 (JS에서 찾을 용도) |
| `id="add-btn"` | 추가 버튼 (JS에서 클릭 이벤트 연결) |
| `id="todo-list"` | 할 일 목록이 들어갈 빈 ul 태그 |
| `<script src="app.js">` | **🔗 app.js 파일 연결!** |
...
<input type="text" id="name" autocomplete="off" placeholder="이름 입력...">
    ...
    <script>
        ...
        if (savedName) { //savedname에 값이 있으면(true) null이면 false
            result.textContent =savedName;  //처음에 널로 넘어가니 false라 실행되지 않음, 두번째부터는 값이 들어오므로 새로고침때마다 자동실행
        }
